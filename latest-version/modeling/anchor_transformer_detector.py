...
from adet.modeling.dptext_detr.anchor_models import DPText_DETR
...
class TransformerPureDetector(nn.Module):
    ...
    def prepare_targets(self, targets, image_size, image):
        ...
        for idx, targets_per_image in enumerate(targets):
            ...
            # print(h, w)
            # print(image_size)
            # mean = torch.tensor([123.675, 116.280, 103.530])[None,None,:].cuda()
            # std = torch.tensor([58.395, 57.120, 57.375])[None, None,:].cuda()
            # print(image[idx].cpu().numpy().shape)
            # ori_image = (image[idx].permute(1,2,0) * std + mean).cpu().numpy()
            ...
            h, w = image_size
            raw_ctrl_points = raw_ctrl_points.reshape(-1, self.dptext_detr.num_ctrl_points, 2)
            gt_ctrl_points = raw_ctrl_points / \
                torch.as_tensor([w, h], dtype=torch.float, device=self.device)[None, None, :]
            
            make_shrink_map = MakeShrinkMap(shrink_ratio=1)
            instance_mask = []
    ...
    def inference(self, ctrl_point_cls, ctrl_point_coord, image_sizes):
        ...
        for scores_per_image, labels_per_image, ctrl_point_per_image, image_size in zip(
            ...
            selector = scores_per_image >= 0.5#self.test_score_threshold
            # print(selector)
