...
class DeformableTransformer_Det(nn.Module):
    ...
    def forward(self, srcs, masks, pos_embeds, query_embed, one_fourth_feature):
        ...
        for lvl, (src, mask, pos_embed) in enumerate(zip(srcs, masks, pos_embeds)):
            bs, c, h, w = src.shape
            # print(src.shape, pos_embed.shape)
            spatial_shape = (h, w)
        ...
        mask_feature, multiscale_feature = self.feature_fuse(encoder_features, one_fourth_feature)
        ...
        inter_references, segmentation_map, inter_class = self.decoder_new(
        ...
        return inter_references_out, enc_outputs_class, enc_outputs_anchor_unact, segmentation_map, semantic_seg_mask, inter_class
...
class DeformableTransformerDecoderLayer_Det(nn.Module):
    def __init__(
        ...
        if self.efsa:
            ...
            # self.attn_intra = nn.MultiheadAttention(d_model, n_heads, dropout=dropout, batch_first=True)
...
class DeformableTransformerDecoder_AnchorDet(nn.Module):
    def __init__(
        ...
        # self.text_semantic_embed = None

        self.ref_point_head = MLP(d_model, d_model, d_model, 2)
        self.pos_scale = MLP(d_model, d_model, d_model, 2)
    ...
    def forward_prediction_heads_class(self, output, query_pos, lvl):
        ...
        # output = self.query_pos_norm(output)
        ...
        outputs_class = self.ctrl_point_class[lvl](self.decoder_norm(aggregated_query))#.mean(dim=-2)

        return outputs_class
        

    def forward_prediction_reg_only(self, output, reference_point, query_pos, lvl):
        # output = self.query_pos_norm(output)
        lvl -= 2
        corrd_offset = self.offset_embed[lvl](output)

        reference_point_updated = (inverse_sigmoid(reference_point.detach()) + corrd_offset).sigmoid()

        return reference_point_updated
    

    def forward_prediction_reg_and_seg(self, output, mask_features_ori, query_pos, semantic_mask, lvl):
        b, nq, n_ctrl, h = output.shape
        ...
        # outputs_class = self.ctrl_point_class(aggregated_query)

        mask_features = self.mask_feature_proj(mask_features_ori)
        mask_embed = self.mask_embed[lvl](self.decoder_norm(aggregated_query))
        outputs_mask = torch.einsum("bqc,bchw->bqhw", mask_embed, mask_features)#.detach()

        # print(outputs_mask.shape, semantic_mask.shape)
        # if lvl > 0:
        #     outputs_mask = intermediate_segmentation_map[lvl-1].sigmoid().detach() * outputs_mask
        ...
        # pixel_feature = torch.sum(outputs_anchor_weights_map.detach().permute(0,1,4,2,3) * mask_features[:,None,:,:,:].detach(), dim=(-1,-2))
        # print(pixel_feature.shape)
    
        anchor_offset = self.anchor_offset_embed[lvl](output)
        reference_point = (inverse_sigmoid(anchor_point.unsqueeze(2)) + anchor_offset).sigmoid()
        
        # corrd_offset = self.offset_embed[lvl](output)
        # reference_point = (inverse_sigmoid(reference_point_tmp) + corrd_offset).sigmoid()

        with torch.no_grad():
            pixel_feature = torch.sum(F.softmax(outputs_mask.detach(), dim=1).unsqueeze(2) * mask_features_ori.detach().unsqueeze(1), dim=(-1,-2))
            # # for i in range(16):
        output = self.post_norm(output + self.linear_proj(pixel_feature)[:,:,None])

        return output, reference_point, anchor_point, outputs_mask
    ...
    def forward(
        ...
        for lid, layer in enumerate(self.layers):
            ...
            if lid == 2:
                output = output + self.ctrl_point_embed.weight[None,None,:,:]
            ...
            if lid == 0:
                reference_points_input = reference_points[:, :, :, None].repeat(1,1,1,4,1)
            else:
            ...
            query_pos = self.ref_point_head(query_pos) * self.pos_scale(output)
            
            
            output = layer(
                output,# bs, n_q, 16, 256
                query_pos, #  (bs, nq, 16, dim)
                reference_points_input, #(bs, nq, 16, 4, 2)
                src, #(bs, map_size, 256)
                src_spatial_shapes,
                src_level_start_index,
                src_padding_mask,
            ...
            if lid < 2:
                output, reference_points, anchor_point, segmentation_map = self.forward_prediction_reg_and_seg(output, fused_feature, query_pos, semantic_mask, lid)
            else:
                reference_points = self.forward_prediction_reg_only(output, intermediate_reference_points[-1], query_pos, lid)
                
            output_class = self.forward_prediction_heads_class(output, query_pos, lid)
            

            if self.return_intermediate and lid < 2:
            ...
            elif self.return_intermediate and lid >= 2:
                # intermediate.append(output)
                intermediate_reference_points.append(reference_points)
                # intermediate_query_pos.append(query_pos)
                # intermediate_anchor_point.append(intermediate_anchor_point[-1])
                intermediate_segmentation_map.append(intermediate_segmentation_map[-1])

            intermediate_class.append(output_class)

        if self.return_intermediate:
            return torch.stack(intermediate_reference_points), torch.stack(intermediate_segmentation_map), torch.stack(intermediate_class)
