...
class SetCriterion(nn.Module):
    ...
    def loss_ctrl_points(self, outputs, targets, indices, num_inst):
        ...
        # pre_defined_anchor_grids = torch.stack((x, y), 2).transpose(0,1).float().to('cuda')
        # print(pre_defined_anchor_grids.shape, target_segmentation_masks.shape)
        # tgt_anchor_weights_map = F.softmax(target_segmentation_masks.reshape(n, h*w), dim=-1).reshape(n,h,w).unsqueeze(-1)
        # mask_points = torch.count_nonzero(target_segmentation_masks, dim=(1,2))
        # target_anchor_points = (pre_defined_anchor_grids.unsqueeze(0) * tgt_anchor_weights_map).sum(dim=(1,2))
        ...
        losses = {'loss_ctrl_points': loss_ctrl_points / num_inst, 'loss_anchor_weights_map_dice':loss_dice / num_inst,\
                'loss_anchor_weights_map_ce':loss_mask_ce / num_inst}
        return losses
