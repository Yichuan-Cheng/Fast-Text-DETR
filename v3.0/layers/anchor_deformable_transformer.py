...
class DeformableTransformerDecoder_AnchorDet(nn.Module):
    ...
    def forward_prediction_reg_only(self, output, reference_point, query_pos, lvl):
        ...
        lvl -= 3
        corrd_offset = self.anchor_offset_embed[lvl](output)

        reference_point = (inverse_sigmoid(reference_point.detach()) + corrd_offset).sigmoid()

        return reference_point
    

    def forward_prediction_reg_and_seg(self, output, mask_features, query_pos, lvl):
        ...
        mask_embed = self.mask_embed[lvl](decoder_output)
        ...
        reference_point = (inverse_sigmoid(anchor_point.unsqueeze(2)) + corrd_offset).sigmoid()
    ...
    def forward(
        ...
        for lid, layer in enumerate(self.layers):
            ...
            if lid < 3:
                reference_points, anchor_point, segmentation_map = self.forward_prediction_reg_and_seg(output, fused_feature, query_pos, lid)
            else:
                reference_points = self.forward_prediction_reg_only(output, intermediate_reference_points[-1], query_pos, lid)


            if self.return_intermediate and lid < 3:
            ...
            elif self.return_intermediate and lid >= 3:
                intermediate.append(output)
                intermediate_reference_points.append(reference_points)
                intermediate_query_pos.append(query_pos)
