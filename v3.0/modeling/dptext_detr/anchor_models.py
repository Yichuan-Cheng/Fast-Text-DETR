...
class DPText_DETR(nn.Module):
    def __init__(self, cfg, backbone):
        ...
        # self.instance_embd = nn.Embedding(100, self.d_model)
        ...
        self.ctrl_point_class = nn.Linear(self.d_model, self.num_classes)#MLP(self.d_model, self.d_model, self.num_classes, 2)
        ...
        self.offset_embed = nn.ModuleList([self.offset_embed for _ in range(3)])
        self.anchor_offset_embed = nn.ModuleList([self.anchor_offset_embed for _ in range(3)])
        self.mask_embed = nn.ModuleList([self.mask_embed for _ in range(3)])
        # if self.epqm:
    ...
    def forward(self, samples: NestedTensor):
        ...
        ctrl_point_embed = self.ctrl_point_embed.weight[None,:,:].repeat(self.num_proposals, 1, 1) #+ self.instance_embd.weight[:,None,:]

        hs, init_reference, inter_references, enc_outputs_class, enc_outputs_coord_unact, \
        segmentation_maps, anchor_point, inter_query_pos, semantic_seg_mask = self.transformer(
        ...
        for i, output in enumerate(hs):
            ...
            outputs_class = self.forward_prediction_heads_class(output, inter_query_pos[i], i)
        ...
        # print(self.offset_embed[0].layers[-1].weight.data, self.offset_embed[1].layers[-1].weight.data)
        # print(outputs_seg_mask.shape, outputs_coord.shape)
        if self.aux_loss:
            out['aux_outputs'] = self._set_aux_loss(outputs_class, outputs_coord, outputs_anchor, outputs_seg_mask)
            # out['aux_outputs_reg'] = self._set_aux_loss_reg_only(outputs_class, outputs_coord)
    ...
    @torch.jit.unused
    def _set_aux_loss_reg_only(self, outputs_class, outputs_coord, outputs_anchor, outputs_seg_mask):
        # this is a workaround to make torchscript happy, as torchscript
        # doesn't support dictionary with non-homogeneous values, such
        # as a dict having both a Tensor and a list.
        return [
            {'pred_logits': a, 'pred_ctrl_points': b} \
            for a, b in zip(outputs_class[3:-1], outputs_coord[3:-1])
        ]